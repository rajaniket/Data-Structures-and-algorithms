...
int index_lb=LB-arr;
cout<<"Lower Bound index is "<<index_lb<<" and the number is "<<arr[index_lb]<<endl; // lower bond means just less than equal to key
int* UB=upper_bound(arr,arr+n,key);
int index_ub=UB-arr;
if(index_ub==n)
    cout<<"UB is not Possible"<<endl;
else
cout<<"Upper Bound index is "<<index_ub<<" and the number is "<<arr[index_ub]<<endl;  // upper bond means just greater than key
